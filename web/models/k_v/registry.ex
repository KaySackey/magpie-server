defmodule KV.Registry do
	...
	@child KV.MapSupervisor


  @doc """
  EXAMPLE. Return an object to store within the registry
  """
  def _start_child(name) do
  	# Name is the name of this.
  	# Should return a value holding process
  	# Todo: simpler versions of this might just want to return a bare

		# Place value holder under supervision tree
		# This ensures that its crashes will not affect us
    {:ok, bucket} = @child.start_link()
  ...
  def start_link(registry_name) do
    {:ok, pid} = GenServer.start_link(__MODULE__, [registry_name], [name: registry_name])
  end
  ...
  def init(options) do
    	[registry_name] = options
  		...
  		state = %Registry{name: registry_name}
			...
			Logger.debug("Initialized KV Registry with #{inspect(state)}")
			{:ok, state}
  end
  ...
  def lookup(server, name) do
    ...
    Logger.debug "lookup: " <> inspect(reply)
    reply
  end
  ...
  def values(server, name) do
    GenServer.call(server, {:values, name})
  end
